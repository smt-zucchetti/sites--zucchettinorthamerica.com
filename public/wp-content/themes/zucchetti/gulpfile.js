...
var processScripts = function(done) {
  ...
  manifest.forEachDependency('js', function(dep) {
      ...
      // check if there are actual bower dependencies.
     if (dep.globs.length) {
          merged.add(
...
gulp.task('icons', function(){
  ...
  return gulp.src(temp + '/icons/**/*.svg')
    ...
    .on('glyphs', function(glyphs, options) {
      ...
      gulp.src(temp + '/styles/core/icons-template.scss') // a template scss file, used to generate the scss code for all the icons
        ...
        .pipe(gulp.dest(temp + '/styles/core/')); 
      })
...
gulp.task('blocks', function() {
  return gulp.src(path.source + 'styles/blocks/*.scss')
    .pipe(sass({
    ...
    .pipe(autoprefixer({
      overrideBrowserslist: ['last 2 versions', 'android 4', 'opera 12'],
      cascade: false
    }))
    // Rename & save into folder named after original file basename
    .pipe(rename(function (file) {
      // file.basename = 'my-block' for example
      // Set dirname to the folder named same as the file basename
      file.dirname = file.basename;
      // Rename the output file to styles.css
      file.basename = 'style';
      file.extname = '.css';
    }))
...
gulp.task('watch', function() {
  ...
  gulp.watch([path.source + 'styles/**/*', '!'+ path.source + 'styles/blocks/*'], gulp.series('styles'));
  gulp.watch([path.source + 'styles/blocks/*'], gulp.series('blocks'));
  ...
  gulp.watch([path.source + 'icons/**/*'],  gulp.series('icons','styles','fonts'));
  gulp.watch(['bower.json', 'assets/manifest.json'], gulp.series('build'));
  gulp.watch(['blocks/**/*.js']).on('change', browserSync.reload);
...
gulp.task(
  'build',
  gulp.series('styles', 'scripts', gulp.parallel('fonts', 'images', 'icons'))
);
